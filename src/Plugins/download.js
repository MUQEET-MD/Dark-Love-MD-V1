...
module.exports = [ {
  ...
  operate: async ({ m, args, prefix, command, Kango, reply, mess, isUrl }) => {
    if (!args[0])
      return reply(`*GitHub link to clone?*\nExample :\n${prefix}${command} https://github.com/ayodejibot/Dark-Love-MD-V1`);
 ...
 • *URL:* ${json.url}`;
  ...
  operate: async ({ m, reply, args, Kango }) => {
    ...
    try {
      ...
      const caption =
        `🎶 *Dark-Love-MD-V1 - SPOTIFY DOWNLOADER*\n` +
        `🎧 *Title:* ${title}\n` +
        `🧑‍🎤 *Artist:* ${artist}\n` +
...
},
{
  ...
  operate: async ({ Kango, m, reply }) => {
    try {
      ...
      if (!mime.startsWith("image/") || !/\/(jpe?g|jpeg)/.test(mime)) {
        return reply("❌ Please reply to a valid image (jpg, webp, jpeg, jpeg).");
      }
